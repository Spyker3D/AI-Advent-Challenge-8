...
Операции используют `execFile` с массивом аргументов, фиксированной командой `git`, timeout и лимитом вывода. Произвольные shell-команды не принимаются, refs валидируются. Weather scheduler по умолчанию отключён и не изменяет `weather-history.json`; для явного включения используется `MCP_ENABLE_WEATHER=true`.
...
Если позже потребуется фоновый сбор погоды, перед запуском MCP задайте:

```powershell
$env:MCP_ENABLE_WEATHER = "true"
node mcp-server/server.js
```
