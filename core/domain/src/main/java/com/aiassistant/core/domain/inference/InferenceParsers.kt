...
object InferenceParsers {
    ...
    fun presentation(raw: String, expectedAction: IncidentAction): ParseResult<UserFacingIncidentResult> = parse(raw, setOf("title","message","user_action")) { o ->
        o.presentation().also { it.validate(expectedAction) }
    }
    fun monolithic(raw: String): ParseResult<Triple<String, IncidentDecision, UserFacingIncidentResult>> = parse(raw, DECISION_FIELDS + setOf("normalized_summary","title","message","user_action")) { o ->
        ...
        decision.validate()
        Triple(o.nonBlank("normalized_summary"), decision, o.presentation().also { it.validate(decision.action) })
    }
    ...
    private fun JsonObject.presentation() = UserFacingIncidentResult(nonBlank("title"), nonBlank("message"), nonBlank("user_action"))
