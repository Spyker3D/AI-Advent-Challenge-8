...
object InferenceSchemas {
    val MONOLITHIC = schema("normalized_summary","category","severity","action","confidence","evidence_state","supporting_evidence","contradicting_evidence","title","message","user_action", properties = """"normalized_summary":{"type":"string"},${decisionProperties()},"title":{"type":"string"},"message":{"type":"string"},"user_action":{"type":"string"}""")
    ...
    private inline fun <reified T : Enum<T>> enumValues(): String =
        kotlin.enumValues<T>().joinToString { "\"${it.name}\"" }
}
