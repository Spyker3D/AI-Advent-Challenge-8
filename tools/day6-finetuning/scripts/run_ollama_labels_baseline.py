...
LABELS_SYSTEM_PROMPT = """Выбери ровно одну метку из списка:
NETWORK_UNAVAILABLE
OPENAI_RATE_LIMIT
OPENAI_TIMEOUT
EMPTY_AI_RESPONSE
...
Верни только метку без объяснений."""
...
RESULTS_PATH = REPORTS_DIR / "labels_baseline_results.jsonl"
SUMMARY_PATH = REPORTS_DIR / "labels_baseline_summary.txt"
...
def main() -> None:
    ...
    print(f"System prompt: {LABELS_SYSTEM_PROMPT!r}")
    ...
    for index, record in enumerate(
        ...
        raw_response = call_ollama(
            system_prompt=LABELS_SYSTEM_PROMPT,
            user_text=user_text,
        )
        ...
        result = {
            "index": index,
            "model": MODEL_NAME,
            "evaluation_mode": "labels_system_prompt",
            "system_prompt": LABELS_SYSTEM_PROMPT,
            "user": user_text,
            "expected": expected,
    ...
    summary_lines = [
        f"Model: {MODEL_NAME}",
        "Evaluation mode: labels system prompt",
        f"System prompt: {LABELS_SYSTEM_PROMPT}",
        f"Dataset: {EVAL_PATH}",
        f"Examples: {total}",
        (
            "Exact accuracy: "
