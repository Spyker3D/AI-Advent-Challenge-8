...
EVAL_PATH = PROJECT_DIR / "data" / "eval.jsonl"
REPORTS_DIR = PROJECT_DIR / "reports"
RESULTS_PATH = REPORTS_DIR / "baseline_results.jsonl"
SUMMARY_PATH = REPORTS_DIR / "baseline_summary.txt"
...
def main() -> None:
    ...
    if len(eval_records) < TEST_LIMIT:
        raise ValueError(
            f"В eval.jsonl только {len(eval_records)} примеров, "
    ...
    for index, record in enumerate(selected_records, start=1):
        ...
        print(f"[{index}/{TEST_LIMIT}] {user_text}")
